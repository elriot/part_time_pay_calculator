...
export default function ShiftRow({
  ...
  const job = jobs.find((j) => j.id === (shift.jobId ?? shift.job));
  const effectiveRate = Number(job?.rate ?? 0);
  ...
  return (
    <tr className={`border-t border-gray-200 dark:border-gray-800 ${isDragging ? "opacity-50" : ""} ${isOver ? "outline outline-2 outline-blue-400/50" : ""}`} {...rowDragProps}>
      ...
      <td className="py-1.5 pr-2">
        <input type="date" className={inputBase} value={shift.date} onChange={(e) => onChange({ date: e.target.value })} />
      </td>
      ...
      <td className="py-1.5 pr-2">
        <input type="time" className={inputBase} value={shift.start} onChange={(e) => onChange({ start: e.target.value })} />
      </td>
      <td className="py-1.5 pr-2">
      ...
      <td className="py-1.5 pr-2 text-right">
        <button
          type="button" onClick={onRemove} aria-label={t("delete")} title={t("delete")}
